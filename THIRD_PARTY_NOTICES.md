# Third-Party Norices

ShowHost uses the following third-party software and assets.

## Serilog

https://serilog.net/

Serilog is used for server-side logging.

License: Apache License 2.0

A copy of the Apache Licnese 2.0 is included in:

[LICENSES/Apache-2.0_Serilog.txt](LICENSES/Apache-2.0_Serilog.txt)

The original license text is also available at:

https://www.apache.org/licenses/LICENSE-2.0

## Feather Icons

https://feathericons.com/

Feather Icons are used throughout the client application.

License: MIT License

The license is included in:

[LICENSES/MIT_FeatherIcons.txt](LICENSES/MIT_FeatherIcons.txt)

## Besley

https://indestructibletype.com/

Besley is used as a font in the client application.

Designed by Owen Earl / Indestructible Type.

License: SIL Open Font License 1.1

The license is included with the font in:

[Client/Assets/Fonts/Besley/LICENSE.md](Client/Assets/Fonts/Besley/LICENSE.md)

## Word List from ACA

https://github.com/csmith/aca

The word lists are used to generate default game names on the server.

License: MIT License

The license is included with the word lists in:

[Server/Keys/Words/LICENCE](Server/Keys/Words/LICENCE)
